...
2. **Evaluator & Generator** runs every 90 minutes and freezes at most the first five eligible rows per execution. It processes that fixed batch sequentially, with an independent append-winner claim, evaluation, generation, guarded commit, and persistence confirmation for each row. It routes to `ready_to_apply`, `review_needed`, or `skip`; provider and source failures become observable `error`/`unavailable` conditions and do not stop later rows. A ready row requires a current, validated application pack and message. Each row allows one initial model request and, only after deterministic rejection, one bounded repair request.
