...
Every Alerter & Mover Google Sheets read permits at most one automatic retry and waits a full 65-second quota window, never five seconds. The 300-second execution reserves at least 150 seconds before receipt/provider commit work. A full movement-plus-alert path uses at most ten Google Sheets read requests; a movement-plus-recovery path uses at most six. The final sanitized summary reports store/status counts, movement/outcome counts, alert delivery/reconciliation classes, Sheet reads, quota retries, provider classifications, and bounded error categories.
