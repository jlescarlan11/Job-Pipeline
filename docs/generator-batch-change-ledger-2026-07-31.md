...
| --- | --- | --- | --- | --- |
| `config/runtime.json`, `src/runtime.mjs` | #47/#49 | Set cap five; validate integer range 1–5 and the production-safe 20-second per-candidate Sheet pacing floor while preserving schedule, timeout, lease, and retries. Runtime callers, schedules, deployment calculations, and boundary tests inspected. | High | REVIEWED_AFTER_FIX |
| `config/groq-provider-policy.json`, `src/groq-provider.mjs` | #47/#49 | Split initial/repair models; bound prompts, per-model quotas, shared-model quotas, per-minute pacing, daily use, Sheet pacing, and timeout. Build, benchmark, pricing, lifecycle, and failure boundaries inspected. | High | REVIEWED_AFTER_FIX |
...
| `workflows/generator.json` | #48/#49 | Deterministic generated 47-node artifact for the sequential lifecycle, including a quota-aware delay after every handled candidate. Source/artifact drift, n8n 2.32.6 import/export, graph topology, settings, env bindings, retry flags, and endpoint surface inspected. | High | REVIEWED_AFTER_FIX |
